...
if [[ "${anansiblevar_VARIABLE_USER}" =~ "_username" ]]; then
     anansiblevar_VARIABLE_HOST=$(aws_dynamodb_anansiblevar_search_exact.sh -c ${COUNTRY_CODE} -k ID -v "${anansiblevar_VARIABLE_USER//username/host}" | awk -F\= '{print $1}')
elif [[ "${anansiblevar_VARIABLE_USER}" =~ "_user" ]]; then
     anansiblevar_VARIABLE_HOST=$(aws_dynamodb_anansiblevar_search_exact.sh -c ${COUNTRY_CODE} -k ID -v "${anansiblevar_VARIABLE_USER//user/host}" | awk -F\= '{print $1}')
fi
#echo "${anansiblevar_USER}=${anansiblevar_PASSWORD}"

if ! [[ -z "${anansiblevar_VARIABLE_USER}" ]]; then
echo "${anansiblevar_VARIABLE_HOST}:${NEW_RDS_HOST}"
echo "python3 dbacli variables put_variable --name ${anansiblevar_VARIABLE_HOST} --value ${NEW_RDS_HOST} --country ${COUNTRY_CODE,,}"
fi
done
